...
Circle.prototype.drawBaseCircle = function() {
  ...
  this.ctx.arc(
    ...
    this.canvas.height / 2 - this.percentageWidth / 2,
    0,
    Math.PI * 2
  )
...
Circle.prototype.drawCurrentProgress = function() {
  ...
  if (this.currentArc >= this.endArc) {
    ...
    this.ctx.arc(
      ...
      this.canvas.height / 2,
      this.canvas.height / 2 - this.percentageWidth / 2,
      startArc.toFixed(2) - 0.05,
      this.currentArc.toFixed(2)
    )
    this.ctx.stroke()
  }
  // 做动画
...
Circle.prototype.drawText = function(text) {
  ...
  this.ctx.clearRect(
    ...
    this.canvas.height / 2 - this.fontSize,
    textWidth * 1.1,
    this.fontSize * 2
  )
  //设置文本的垂直对齐方式
